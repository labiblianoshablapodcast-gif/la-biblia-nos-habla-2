...
export default function BibleAudioControls({language,bookCode,chapter}:{language:"qeqchi"|"rvr60";bookCode?:string;chapter?:number;}){
 ...
 useEffect(()=>{
   setPlaying(false);setProgress(0);setLoading(false);setShowFloating(false);
 ...
 useEffect(()=>{
   const update=()=>{
     ...
     if(!card){setShowFloating(false);return;}
     setShowFloating(card.getBoundingClientRect().bottom<0);
   };
 ...
 const controls=(floating=false)=><div className={floating?"bibleAudioControls bibleAudioFloatingControls":"bibleAudioControls"}>
 ...
 <section ref={cardRef} className="bibleAudioCard" aria-label={`Audio bíblico ${label}`}>
   ...
   {ready&&<audio key={payload!.audioUrl} ref={audioRef} src={payload!.audioUrl} preload="metadata" onPlay={()=>setPlaying(true)} onPause={()=>setPlaying(false)} onLoadedMetadata={()=>{const audio=audioRef.current;if(audio){audio.playbackRate=speed;audio.currentTime=startOffset(audio);syncVerse();}}} onError={()=>{setPlaying(false);setPayload({ok:false,error:"No pudimos cargar el audio. Recarga la página para intentarlo de nuevo."});}} onTimeUpdate={syncVerse} onSeeked={syncVerse} onEnded={()=>{setPlaying(false);emitVerse(null);}}/>}
   {controls()}
   <p className="bibleAudioStatus">{loading?"Conectando el audio RVR1960…":ready?timings.length?"Audio RVR1960 conectado · el texto seguirá el versículo automáticamente.":"Audio RVR1960 conectado.":payload?.error||"Audio no disponible para este capítulo."}</p>
 </section>
