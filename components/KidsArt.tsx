...
export default function KidsArt({scene=0}:{scene?:number}){
 return <svg className={styles.art} viewBox="0 0 800 440" role="img" aria-label={["David cuida las ovejas en el campo","David observa el campamento y a un guerrero distante","David elige piedras junto al arroyo","David recuerda la ayuda de Dios","El ejército celebra la victoria sin mostrar violencia","Lumi invita a orar y pedir ayuda"][scene]}>
  <rect width="800" height="440" fill={scene===3?"#d9e9f3":"#d9f0f0"}/><circle cx="667" cy="77" r="40" fill="#f7ca64"/>
  ...
  {scene===2&&<><path d="M800 291Q620 319 506 440H263Q524 300 800 265Z" fill="#8ccee2"/>{[0,1,2,3,4].map(i=><ellipse key={i} cx={380+i*37} cy={386+(i%2)*15} rx="14" ry="10" fill="#e6ddc8" stroke="#697c78" strokeWidth="2"/>)}</>}
  {(scene===1||scene===3)&&<g transform="translate(600 162)"><path d="M-50 124L0 48L53 124Z" fill="#cda86e"/><path d="M-8 124V77L13 124Z" fill="#6a654e"/><g transform="translate(87 -32)"><circle cy="30" r="17" fill="#b8805d"/><path d="M-23 48H23L33 121H-33Z" fill="#768296"/><path d="M-16 119V153M16 119V153M-24 66L-43 105" stroke="#3c5263" strokeWidth="12" strokeLinecap="round"/><ellipse cx="30" cy="89" rx="20" ry="30" fill="#b69c69" stroke="#746d52" strokeWidth="4"/></g></g>}
  <g className={styles.david} transform={`translate(${scene===2?235:scene===5?480:310} 161)`}>
  ...
  {(scene===0||scene===5)&&[0,1].map(i=><g key={i} transform={`translate(${scene===5?185+i*120:490+i*140} ${340+i*18})`}><path d="M-24 19V49M20 19V49" stroke="#5c625c" strokeWidth="8"/><ellipse rx="46" ry="30" fill="#fff8e5" stroke="#d2cebc" strokeWidth="3"/><circle cx="38" cy="-2" r="21" fill="#e2c8a7"/><circle cx="43" cy="-5" r="3" fill="#3f514d"/></g>)}
  {scene===4&&<g fill="#f5c65d">{[0,1,2,3,4].map(i=><path key={i} transform={`translate(${130+i*133} ${100+(i%2)*36})`} d="M0 -15L5 -5L16 -3L7 5L9 16L0 10L-9 16L-7 5L-16 -3L-5 -5Z"/>)}</g>}
