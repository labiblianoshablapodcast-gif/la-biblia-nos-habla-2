...
function sceneForTitle(title:string){
 const v=title.toLowerCase();
 if(v.includes("nacimiento de jesús")) return "/images/qeqchi/nativity-pilot.svg";
 return "";
}
...
export default function QeqchiScriptureVideos(){
 ...
 const scene=useMemo(()=>sceneForTitle(title),[title]);
 ...
 return <section className="shell">
  ...
  {!loading&&!error&&videos.length>0&&<div className="layout">
   <article className="playerCard">
    <div className="stage">
      <video key={current} controls playsInline preload="metadata"><source src={current}/></video>
      <div className={`cinemaFrame ${scene?"hasScene":""}`} style={scene?{backgroundImage:`linear-gradient(to top,rgba(0,0,0,.55),rgba(0,0,0,.04) 45%),url(${scene})`}:undefined} aria-hidden="true">
        {!scene&&<div className="glow"/>}
