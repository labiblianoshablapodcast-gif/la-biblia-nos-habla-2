 ...
 return <section className="shell">
  ...
  {!loading&&!error&&videos.length>0&&<div className="layout">
   <article className="playerCard">
    <div className="stage">
      <video ref={mediaRef} key={current} className="mediaVideo" playsInline preload="auto" onClick={toggle} onLoadedMetadata={e=>setDuration(e.currentTarget.duration)} onTimeUpdate={e=>setTime(e.currentTarget.currentTime)} onPlay={()=>setPlaying(true)} onPause={()=>setPlaying(false)} onEnded={()=>setPlaying(false)}><source src={current} type="video/mp4"/></video>
